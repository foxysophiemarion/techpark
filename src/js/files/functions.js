...
export function showMore() {
	window.addEventListener("load", function (e) {
		...
		function updateTabContentHeights() {
			const showMoreBlocks = document.querySelectorAll('[data-showmore]');
			showMoreBlocks.forEach(block => {
				const content = block.querySelector('[data-showmore-content]');
				if (content) {
					content.style.height = '152px'; // Установить при необходимости
					content.style.overflow = 'hidden'; // Установить при необходимости
				}
			});
		}

		// Вызовите эту функцию при переключении табов
		document.addEventListener("click", (e) => {
			if (e.target.closest('.gallery-reviews__item')) {
				updateTabContentHeights();
			}
		});
	});
}
